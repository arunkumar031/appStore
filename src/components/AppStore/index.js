...
class AppStore extends Component {
  ...
  filteredTabAppsList = () => {
    ...
    const filteredTabAppsList = appsList.filter(
      each => each.category === activeTabId,
    )
    return filteredTabAppsList
  }

  render() {
    const {activeTabId, searchInput} = this.state
    const filteredTabAppsList = this.filteredTabAppsList()
    console.log(filteredTabAppsList)
    const filteredAppsList = filteredTabAppsList.filter(each =>
